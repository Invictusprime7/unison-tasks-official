...
export const agencyTemplates: LayoutTemplate[] = [
  {
    ...
    code: wrapInHtmlDoc(`
<nav class="fixed top-0 left-0 right-0 z-50 px-6 py-4 bg-slate-950/90 backdrop-blur-lg"><div class="max-w-7xl mx-auto flex items-center justify-between"><a href="#" class="text-xl font-bold text-violet-500">Amplify</a><div class="hidden md:flex items-center gap-6"><a href="#" class="text-sm text-slate-300 hover:text-white">Services</a><a href="#" class="text-sm text-slate-300 hover:text-white">Work</a><a href="#" class="text-sm text-slate-300 hover:text-white">About</a><a href="#" class="text-sm text-slate-300 hover:text-white">Contact</a></div><button class="px-5 py-2 bg-violet-500 text-white text-sm font-medium rounded-full">Let's Talk</button></div></nav>
<section class="min-h-screen flex items-center px-6"><div class="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center"><div><span class="text-violet-400 font-medium">FULL-SERVICE MARKETING AGENCY</span><h1 class="text-5xl md:text-7xl font-bold mt-4 mb-6 leading-tight">Brands that <span class="text-violet-500">demand</span> attention</h1><p class="text-xl text-slate-400 mb-8">We craft strategies that turn audiences into advocates and clicks into customers.</p><div class="flex gap-4"><button class="px-8 py-4 bg-violet-500 text-white font-medium rounded-full">Start a Project</button><button class="px-8 py-4 border border-white/20 rounded-full hover:bg-white/5">Our Work</button></div></div><div class="relative"><div class="absolute -inset-4 bg-gradient-to-r from-violet-500/20 to-pink-500/20 blur-3xl rounded-full"></div><img src="https://images.unsplash.com/photo-1557804506-669a67965ba0?w=500&q=80" alt="" class="relative rounded-2xl"/></div></div></section>
...
<section class="py-24 px-6 bg-gradient-to-r from-violet-600 to-pink-600"><div class="max-w-4xl mx-auto text-center"><h2 class="text-4xl font-bold mb-6">Ready to Amplify Your Brand?</h2><p class="text-white/80 text-xl mb-8">Let's build something remarkable together.</p><button class="px-10 py-4 bg-white text-violet-600 font-bold rounded-full">Start Your Project</button></div></section>
