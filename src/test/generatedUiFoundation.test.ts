...
describe('generated UI foundation', () => {
  ...
  it('builds one canonical prompt directive enumerating every manifest import path', () => {
    ...
    expect(directive).toContain(`runtime components: ${foundation.manifest.motionExports!.components.join(', ')}`);
    expect(directive).toContain(`Type-only exports: ${foundation.manifest.motionExports!.types.join(', ')}`);
