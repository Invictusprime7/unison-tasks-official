-- AI agent infrastructure required by install-system, agent-runner, and bootstrap.
DO $$ BEGIN
...
DO $$ BEGIN
  ALTER TABLE public.ai_plugin_instances ADD CONSTRAINT config_is_object CHECK (jsonb_typeof(config) = 'object');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  ALTER TABLE public.ai_events ADD CONSTRAINT ai_events_intent_nonempty CHECK (length(trim(intent)) > 0);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  ALTER TABLE public.ai_events ADD CONSTRAINT ai_events_claimed_run_fk FOREIGN KEY (claimed_run_id) REFERENCES public.ai_runs(id) ON DELETE SET NULL;
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE POLICY "ai_agent_registry_select_public" ON public.ai_agent_registry FOR SELECT USING (is_active = true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN
  CREATE POLICY "ai_plugin_instances_select_member" ON public.ai_plugin_instances FOR SELECT USING (public.is_business_member(business_id));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN
  CREATE POLICY "ai_plugin_instances_insert_member" ON public.ai_plugin_instances FOR INSERT WITH CHECK (public.is_business_member(business_id));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN
  CREATE POLICY "ai_plugin_instances_update_member" ON public.ai_plugin_instances FOR UPDATE USING (public.is_business_member(business_id));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN
  CREATE POLICY "ai_plugin_instances_delete_member" ON public.ai_plugin_instances FOR DELETE USING (public.is_business_member(business_id));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN
  CREATE POLICY "ai_plugin_state_select_member" ON public.ai_plugin_state FOR SELECT USING (EXISTS (SELECT 1 FROM public.ai_plugin_instances pi WHERE pi.id = plugin_instance_id AND public.is_business_member(pi.business_id)));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN
  CREATE POLICY "ai_plugin_state_modify_service" ON public.ai_plugin_state FOR ALL USING (auth.role() = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN
  CREATE POLICY "ai_events_insert_service" ON public.ai_events FOR INSERT WITH CHECK (auth.role() = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN
  CREATE POLICY "ai_events_select_member" ON public.ai_events FOR SELECT USING (public.is_business_member(business_id));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN
  CREATE POLICY "ai_events_update_service" ON public.ai_events FOR UPDATE USING (auth.role() = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN
  CREATE POLICY "ai_runs_select_member" ON public.ai_runs FOR SELECT USING (public.is_business_member(business_id));
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN
  CREATE POLICY "ai_runs_modify_service" ON public.ai_runs FOR ALL USING (auth.role() = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
CREATE TRIGGER ai_agent_registry_updated_at BEFORE UPDATE ON public.ai_agent_registry FOR EACH ROW EXECUTE FUNCTION public.handle_ai_updated_at();
DROP TRIGGER IF EXISTS ai_plugin_instances_updated_at ON public.ai_plugin_instances;
CREATE TRIGGER ai_plugin_instances_updated_at BEFORE UPDATE ON public.ai_plugin_instances FOR EACH ROW EXECUTE FUNCTION public.handle_ai_updated_at();
DROP TRIGGER IF EXISTS ai_plugin_state_updated_at ON public.ai_plugin_state;
CREATE TRIGGER ai_plugin_state_updated_at BEFORE UPDATE ON public.ai_plugin_state FOR EACH ROW EXECUTE FUNCTION public.handle_ai_updated_at();
...
VALUES ('lead_qualifier', 'Lead Qualification Agent', 'Analyzes incoming form submissions and scores leads automatically', 'free', 'You are a lead qualification agent. Analyze incoming form submissions and score leads. Always respond with valid JSON.', '["crm.lead.create", "notify.team", "pipeline.stage.set"]'::jsonb, '{"settings": {"min_score_for_notification": 70}, "bindings": {}, "capabilities": {}}'::jsonb)
ON CONFLICT (slug) DO UPDATE SET name = EXCLUDED.name, description = EXCLUDED.description, system_prompt = EXCLUDED.system_prompt, allowed_tools = EXCLUDED.allowed_tools, default_config = EXCLUDED.default_config, updated_at = now();