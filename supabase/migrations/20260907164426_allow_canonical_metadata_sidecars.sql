-- SiteBundleSnapshot intentionally seals executable/runtime files only, while
-- site_revisions.vfs_files also persists durable /.unison/* metadata sidecars.
-- Compare the runtime projection at the transaction boundary without dropping
-- those sidecars from revision history or the builder draft projection.
