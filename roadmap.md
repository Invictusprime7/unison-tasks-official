...
- [x] P3 registry-derived Wizard design UI (`src/services/wizardDesignAvailability.ts`, no new registry) — Visual Direction cards, their availability and the Customize Sections pickers are projected from Art Direction Packs + Variant Registry + the 21st coverage gate + selected pages + experience preference. An uncoverable direction renders unavailable with its reason instead of degrading into the global pool; pins that stop being eligible are dropped before launch; pins may be keyed by section type (Wizard) or section instance id and remain the final authority in `wizardDesignIntervention`.
	- Corrected the experience compatibility rule: the preference is a ceiling on required runtime capability, not an exact motion match. Immersive implementations still require the Immersive preference; motion-rich ones stay available under Standard (their motion is token-driven and reduced-motion documented). The previous exact-match rule would have emptied both the Wizard picker and the AI candidate list for every Standard launch.
...
- [x] Task 14 — SiteDesignContract wired into the composition lane: compiled once at the client acceptance point, transported as `SiteDesignContractProjection` (summary + per-page required roles and density budgets), rendered into the canonical prompt as rule 11, and validated on both sides (`projectionDensityIssues`: ceiling hard, floor + required roles advisory). Edge mirror `supabase/functions/_shared/siteDesignContractProjection.ts`, drift-tested. Tests: src/test/siteDesignContractProjection.test.ts (5). wizard-site-composer redeployed.

- [x] Task 15 (plan §7.5) — Composition affinity: `src/sections/compositionAffinity.ts` ranks legal candidates by derived coherence (art-direction family order, industry dialect families + discouraged traits, page role, provenance, optional authored `compositionAffinity` neighbour weights, composition baseline). `selectAffineVariant()` replaces blind seeded rotation in `wizardDesignIntervention.buildActiveVariants` and the `eligible[0]` pick in `compositionToFileSet`; the band always keeps a runner-up so seeds still produce distinct sites. Affinity guides selection only — certification, pack membership, role eligibility and dependencies remain the sole hard gates. Tests: src/test/compositionAffinity.test.ts (7).

- [x] Task 16 (plan §7.7) — Site-wide inheritance edit: `setFamilyVariant` PresentationOp (`src/types/patchPlan.ts` + `semanticPresentationOps.ts`) writes one shared family decision into the sealed activeVariants map and clears page-local `page.variants` overrides so every page inherits, except roles named in `exceptPageRoles`. Refuses an unregistered design or a family the site lacks. Tests: src/test/siteWideInheritanceOps.test.ts (4).
- [x] Task 17 (plan §8.1/8.2/8.4) — Resolved artifact catalog: `ResolvedImplementationContract` now carries derived `creativeAffinity` (industries, discouraged industries, art directions, page intents, interaction tags), `visualSignature` and `certified`, all derived — never hand-maintained. `resolveLegalImplementation(id, usage)` ends legacy fallback authority: fresh generation and AI edits accept only certified non-legacy ids, saved revisions stay readable, migration resolves through the explicit `LEGACY_IMPLEMENTATION_ALIASES` map (empty until a replacement is proven), unknown ids are rejected with a reason instead of silently falling back. `listLegalImplementations(sectionType, {usage, packId, industry})` is the consumer entry point; `setVariant` edits now ask it. Tests: src/test/legalImplementationResolution.test.ts (6).
- [x] Task 18 — Noir Atelier art direction (owner reference image): new `noir-atelier` pack in `src/sections/variants/artDirectionPacks.ts` encoding the reference's dark studio ground, perspective image band, monumental uppercase lockup, warm signal accent, dark service tiles and thin uppercase proof row — ordering certified 21st implementations only (`hero:image-stream` first, `services:product-cards`, `stats:proof-grid`, `navbar:minimal-dark`). First preferred direction for `salon` in `INDUSTRY_TO_PACKS` and `INDUSTRY_CREATIVE_PROFILES`; no theme-preset family attached, so preset-led resolution is unchanged. Memory: mem://design/noir-atelier-reference-direction.
